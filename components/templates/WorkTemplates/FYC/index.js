...
function FYCWorkTemplate({ work, clickPosition, zoomedSize, handleImageClick }) {
    return (
        <div className={`contentWrapper mt-16 ${styles.container}`}>
            ...
            >
                <motion.div
                    key={work._id}
                    className={"flex-col gap-2 items-center text-center mt-10 text-base italic tracking-wider"}
                    variants={textReveal.boxRevealToTop()}
                    transition={{ duration: 2 }}
                >
                    {work.projectSubTitle2.length > 0 ? work.projectSubTitle2?.map(item => (
            ...
            >
                <motion.div
                    ...
                    transition={{ duration: 2 }}
                >
                    {work.projectMainDescription2.length > 0 ? work.projectMainDescription2?.map((item, index) => (
                        item?.children?.map(child => (
                            <p className={index === 1 ? "font-semibold" : ""}>{child.text}</p>
                        ))
                    )) : null}
                </motion.div>
            </motion.div>
            ...
            >
                <motion.div
                    ...
                    transition={{ duration: 2 }}
                >
                    {work.description1.length > 0 ? work.description1?.map((item, index) => (
                        item?.children?.map(child => (
                            <p>{child.text}</p>
                        ))
                    )) : null}
            ...
            >
                <motion.div
                    ...
                    transition={{ duration: 2 }}
                >
                    {work.description2.length > 0 ? work.description2?.map((item, index) => (
                        item?.children?.map(child => (
                            <p>{child.text}</p>
                        ))
                    )) : null}
...
export default FYCWorkTemplate
