...
function SlideshowButtons(props) {
    ...
    return (
        <div
            ...
            <AnimatePresence>
                <motion.div
                    className={`__link ${styles.buttonWrapper} ${(mousePositionOverSlider === ALIGNMENT.LEFT || (isSlidable && isZoomed)) ? "flex" : "hidden"}`}
            ...
            <AnimatePresence>
                <motion.div
                    className={`__link ${styles.buttonWrapper} ${(mousePositionOverSlider === ALIGNMENT.RIGHT || (isSlidable && isZoomed)) ? "flex" : "hidden"}`}
                    variants={fade.fadeIn(0.5)}
